...
class CustomAuthManager {
  ...
  Future signOut() async {
    ...
    haulageCompanyAuthUserSubject.add(
      HaulageCompanyAuthUser(loggedIn: false),
    );
    persistAuthData();
  }

  Future<HaulageCompanyAuthUser?> signIn({
    String? authenticationToken,
    String? refreshToken,
    DateTime? tokenExpiration,
    String? authUid,
  ...
  HaulageCompanyAuthUser? _updateCurrentUser({
    String? authenticationToken,
    String? refreshToken,
    DateTime? tokenExpiration,
    String? authUid,
    ...
    final updatedUser = HaulageCompanyAuthUser(
      loggedIn: true,
      uid: authUid,
      userData: userData,
    );
    haulageCompanyAuthUserSubject.add(updatedUser);
    persistAuthData();
    return updatedUser;
  }
  ...
  Future initialize() async {
    ...
    try {
      ...
      userData = _prefs.getString(_kUserDataKey) != null
          ? AuthResponseDTOStruct.fromSerializableMap(
...
HaulageCompanyAuthUser? currentUser;
